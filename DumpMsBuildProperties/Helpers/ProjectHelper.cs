        ...
        public static IEnumerable<Project> GetProjects(string solutionFile, string configurationName)
        {
            ...
            using (var streamReader = new StreamReader(solutionFile))
            {
                ...
                for (int i = 0; i < array.Length; i++)
                {
                    ...
                    if (!ObjectHelper.AreEqual(ProjectTypePropertyInfo.GetValue(projectInSolution, null), KnownToBeMsBuildFormat))
                    {
                        continue;
                    }

                    var relativePath = (string)RelativePathPropertyInfo.GetValue(projectInSolution, null);
